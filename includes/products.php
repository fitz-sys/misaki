<?php
// FIX: I-set ang timezone sa Pilipinas para magtugma ang PHP at MySQL
...
function fetch_products($visibleOnly=true){
  ...
  foreach($rows as &$r){
    ...
    $product_time = strtotime($r['createdAt']);
    $age_seconds  = $now - $product_time;
    
    // FIX: Kung advance ang MySQL clock kesa sa PHP, maiiwasan ang negative seconds
    if ($age_seconds < 0) {
        $age_seconds = 0;
    }
    ...
    // 1. "New" Badge: Valid ONLY within the first 2 minutes (120 seconds) for testing
    if ($age_seconds <= 120) {
        $badges[] = 'New';
    }
    
    // 2. "Bestseller" Badge: Valid if sales >= 15 AND within the first 2 minutes (120 seconds) for testing
    if ($age_seconds <= 120 && (int)$r['sales'] >= 15) {
        $badges[] = 'Bestseller';
    }
    
    // 3. Stacking: Pagsasamahin ang mga pumasang badge (e.g., "New / Bestseller")
    $r['badge'] = implode(' / ', $badges); 
  }
  return $rows;
}
