...
if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  elseif(!filter_var($email,FILTER_VALIDATE_EMAIL)) $err = 'Please enter a valid email.';
    ...
    try{ register_user($email,$pass,$name ?: 'Customer'); header('Location: '.$next); exit; }
    catch(Throwable $e){ $err = 'Email already registered.'; }
  }
}
...
<div class="page-pad">
  <section class="container auth-wrap reveal">
    ...
    <form method="post" class="auth-form">
      ...
      <label>Password (6+ chars) <input name="password" type="password" required minlength="6"></label>
      <button class="btn btn-ink" type="submit">Create account</button>
    </form>
    <p style="margin-top:20px;font-size:.85rem">Already have an account?
      <a href="login.php?next=<?= urlencode($next) ?>" style="text-decoration:underline">Sign in</a>
    </p>
  </section>
</div>
