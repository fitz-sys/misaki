...
<details class="admin-card">
  ...
  <form method="post" class="admin-form">
    ...
    <label>Name<input name="name" required placeholder="e.g. Ribbon Wrap"></label>
    ...
    <button class="btn btn-ink" type="submit">Create add-on</button>
  </form>
</details>
...
<table class="admin-table">
  ...
  <?php foreach($rows as $a): ?>
    <tr>
      ...
      <td>
        <span class="pill <?= $a['is_active']?'on':'off' ?>">
      ...
      <td>
        <details>
          ...
          <form method="post" class="admin-form" style="margin-top:12px">
            ...
            <label>Name<input name="name" value="<?= htmlspecialchars($a['name']) ?>" required></label>
            <label>Price (₱)<input type="number" step="0.01" min="0" name="price" value="<?= $a['price'] ?>" required></label>
            <label class="checkbox">
              <input type="checkbox" name="is_active" <?= $a['is_active']?'checked':'' ?>> Active
            </label>
